...
class SmsLogModel extends PublicModel
{
    ...
    public function addSms($action = '', $mobile = '', $code = '999999')
    {
        switch ($action) {
            case 'login':
                $sms_content = "【孜尔课堂直播】本次验证码：" . $code . "（" . $time . "分钟内有效），如非本人操作，请忽略本短信";
                break;
            case 'register':
                $sms_content = "【孜尔视频直播】本次验证码：" . $code . "（" . $time . "分钟内有效），如非本人操作，请忽略本短信";
                break;
            case 'findpwd':
                $sms_content = "【孜尔视频直播】本次验证码：" . $code . "（" . $time . "分钟内有效），如非本人操作，请忽略本短信";
                break;
        ...
        $data['zl_use'] = 0;//使用状态
        $data['zn_exp_date'] = $exp_time;//失效期时间（10分钟）
