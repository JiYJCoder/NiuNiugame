...
class Category {
    ...
    public function __construct($model = '', $fields = array(),$funM='') {
        if (is_string($model) && (!empty($model))) {
			if($funM){
				if (!$this->model = call_user_func_array($funM,array($model)) )
...
//		die();
	...
	// 获取节点下的所有子节点
	public function get_child($fid,$level=10,$sql="zl_visible=1"){
		return $fid.$this->_get_child($fid,$level,$sql);
	}
	private function _get_child($fid,$level=10,$sql="zl_visible=1"){	
		//防止无限递归
		if($level<=0){return '';}
		$lcids = '';
		//获取每个节点
		if($sql){
			 $sqlcon=$sql." and ".$this->fields['fid']."=".intval($fid);
		}else{
			 $sqlcon=$this->fields['fid']."=".intval($fid);
		}
		$ids = $this->model->field("`".$this->fields['cid']."`,`".$this->fields['fid']."`")->where($sqlcon)->order($this->fields['cid']." asc")->select();
		//得到每个节点
		if($ids){
			foreach($ids as $key=>$value){
				$lcids.=",".$value["id"];
				//再次调用函数
				$lcids.=$this->_get_child($value['id'],$level-1,$sql);
			}			
		}
		return $lcids;
	}
	...
	// 设置树状结构的纵向层级
	public function set_class($fid,$id,$level=10)
	{	
		//防止无限递归
		if($level<=0){return true;}
		//获取$parent
		$parent = $this->model->where($this->fields['cid']."=" .(int)$fid)->getField($this->fields['fid']);
		//得到每个节点
		if($parent>0)
		{
			//再次调用这个函数获取这个父节点
			$this->set_class($parent,$id,$level-1);
		}else{
			M()->execute("update ".$this->model->getTableName()." set zn_class='".(10-$level)."' where id=".$id);
		}
	}

	//得到根节点
	public function get_top_node($tnid,$level=10)
	{	
		//防止无限递归
		if($level<=0){return false;}
		//获取$parent
		$ladata = $this->model->where($this->fields['cid']."=" .(int)$tnid)->find();
		//得到每个节点
		if($ladata[$this->fields['fid']]==0){
			return $tnid;
		}else{
			//再次调用这个函数获取这个父节点
			return $this->get_top_node($ladata[$this->fields['fid']],$level-1);
		}
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//菜单遍历子节点
	public function join_children($tnid,$orderby,$level,$dealclass=0)
	{	
		//防止无限递归
		if($level<=0){return false;}
		$tree_arr = array();
		...
		//获取$parent
		$condition=array(
			$this->fields['fid']=>$tnid
		);
		//if($dealclass) $this->model->where(array('id'=>$tnid))->save(array("zn_class"=>(10-$level) ));
		if($dealclass) M()->execute("update __PREFIX__".CONTROLLER_TO_TABLE(CONTROLLER_NAME)." set zn_class='".(10-$level)."' where id=".$tnid);
		
		$ladatalist  = empty($orderby) ? $this->model->where($condition)->select() : $this->model->where($condition)->order($orderby)->select();
		//得到每个节点
		if($ladatalist)
		{
			foreach ($ladatalist as $lnKey => $laValue){
				if($dealclass) $this->model->where(array('id'=>$laValue[$this->fields['cid']]))->save(array("zn_class"=>(10-$level-1) ));
				
			    $tree_arr[$laValue[$this->fields['cid']]]=$laValue;
				//再次调用这个函数
				$tree_arr[$laValue[$this->fields['cid']]]['children'] = $this->join_children($laValue[$this->fields['cid']],$orderby,$level-1,$dealclass) ;
			}
		}
		return $tree_arr;
	}
	...
	//菜单往根遍历
	public function lqSysmuenRootfind($tnid,$level=10)
	{	
		//防止无限递归
		if($level<=0){return false;}
		$lcSubstr="";
		...
		if($ladata)
		{
			if( !in_array($ladata["zn_type"],array(0)) ){  //过滤不用的节点id
				 //构建二维表  1:标题 2:url
				 if($ladata["zc_run"]!=''){
				  $lcSubstr.="|@1@|".$ladata["zc_caption"]."|@2@|".$ladata["zc_run"]; 
				 }else{
				  $lcSubstr.="|@1@|".$ladata["zc_caption"]."|@2@|"."nourl";
				 }
			}
			 
			//再次调用这个函数获取这个父节点
			$lcSubstr.=$this->lqSysmuenRootfind($ladata[$this->fields['fid']],$level-1);
