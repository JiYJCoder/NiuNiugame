...
function ucenter_md5($str)
{
    if ('' === $str) {
        return '';
    } else {
        $str = md5($str . AUTH_KEY);
        return $str;
    }
}
...
function lq_cdate($tntime = '', $tntimefull = 1, $tnafterhour = 0)
{
    ...
    if ($tntime) {
        return date($gmdate_format, strtotime($tntime) + $tnafterhour);
    } else {
...
function lq_test($data, $log = 0)
{
    ...
    if ($log == 1) {
        ...
        file_put_contents(WEB_ROOT . "Runtime/txt.txt", "------------------$time start------------------\r\n" . $data . "\r\n\r\n------------------$time end------------------\r\n\r\n\r\n\r\n", FILE_APPEND);
    }
}
...
/*
 * 阿里大于短信通知
 * SMS_11490043  短信验证码模版id
 * $datas 为array()
 */
function lqSendSms($mobile, $datas, $tempId = 'SMS_11490043')
{
    Vendor('Alidayu.TopSdk', '', '.php');
    $appkey = C('ALISMS')['key'];
    $secret = C('ALISMS')['secret'];
    $c = new \TopClient;
    $c->appkey = $appkey;
    $c->secretKey = $secret;
    $c->format = 'json';

    $datas = is_array($datas) ? $datas : array("code" => $datas);
    $req = new \AlibabaAliqinFcSmsNumSendRequest;
    $req->setExtend($datas['code']);
    $req->setSmsType('normal');
    $req->setSmsFreeSignName(C('ALISMS')['sign_name']); //发送的签名
    $req->setSmsParam(json_encode($datas));//根据模板进行填写
 ...
 * 密码base64加密
 */
function api_base64_pwd($user_id, $secret)
{
    return str_replace("=", "00000", base64_encode($user_id . 'NBe6@51=fd$3%6' . $secret . date("Ymd")));
}
 ...
 * 接收base64图片
 */
function save_base64_img($str, $type = 'avatar')
{
    if (!$str) return false;
    $save_to_path = './' . C("UPLOAD_PATH")["folder"] . C("UPLOAD_PATH")["list"][$type] . date("Ymd") . "/";
    if (!is_dir($save_to_path)) mkdir($save_to_path);

    $img_str = explode(";base64,", $str);
    $ext = end(explode("/", $img_str[0]));

    $save_name = time() . lq_random_string(6, 1) . "." . $ext;
    $save_to = $save_to_path . $save_name;

    $img = base64_decode(str_replace($img_str[0] . ";base64,", '', $str));
    //$img = str_replace(' ', '+', $img);
    $a = file_put_contents($save_to, $img);

    if (!empty($a)) {
        $account = 'Nomorl';
        if(session('student_auth')) $account = session('student_auth')['zc_account'] ;
        if(session('teacher_auth')) $account = session('teacher_auth')['zc_account'] ;
        $attachData = array(
...
{
    $Attachment = new \Attachment\Api\AttachmentApi;
    $lc_table = "attachment";
    $lc_folder_path = $data['save_path'];
    $lc_folder_path = substr($lc_folder_path, 1);
    $upfile_data = array(
        'zn_uid' => intval(session('teacher_auth')['id']),
        'zc_account' => $data["zc_account"],
        'zc_table' => $lc_table,
        'zc_controller' => CONTROLLER_NAME,
        'zn_type' => in_array($data['ext'], array('jpg', 'gif', 'png', 'jpeg')) ? 0 : 1,
        'zn_user_type' => 2,
        'zc_original_name' => pathinfo($data["savename"])['extension'],
        'zc_sys_name' => $data["savename"],
        'zc_folder_path' => $lc_folder_path,
        'zc_file_path' => $lc_folder_path . $data["savename"],
        'zc_suffix' => strtolower($data['ext']),
        'zn_size' => $data["size"],
        'zc_folder' => $data['type'],
...
/*
 * 处理阿里云 通过视频id 返回的视频信息
 *  提取视频url和背景图
 */
function getVodInfo($aliInfo)
{
    if (!$aliInfo) return;
    $reArr = array();
    $aliInfo = object2array(json_decode($aliInfo));

    $reArr['CoverURL'] = $aliInfo['VideoBase']['CoverURL'];
    $reArr['PalyInfo'] = array(
        'm3u8' => array(
            'PlayURL' => $aliInfo['PlayInfoList']['PlayInfo'][0]['PlayURL'],
            'Size' => $aliInfo['PlayInfoList']['PlayInfo'][0]['Size']
        ),
        'mp4' => array(
            'PlayURL' => $aliInfo['PlayInfoList']['PlayInfo'][1]['PlayURL'],
            'Size' => $aliInfo['PlayInfoList']['PlayInfo'][1]['Size']
        )
    );
    return $reArr;
}
 ...
 * 处理阿里云 正在直播数据
 */
function getLiving($aliInfo)
{
    if (!$aliInfo) return;
    $reArr = array();
    $aliInfo = object2array(json_decode($aliInfo));

    $streamArr = $aliInfo['OnlineInfo']['LiveStreamOnlineInfo'];
    foreach($streamArr as $key => $val)
    {
        $reArr[] = $val['StreamName'];
    }
...
function get_lesson_cat()
    ...
    // 输入秒数 返回还剩多久
    function lq_surplus_second($data)
    {
        if($data > 86400){
            $day = floor($data / 86400); // 多少天
            $day_data = $data % 86400;
            $hour = floor($day_data / 3600); // 多少小时
            $hour_data = $day_data % 3600;
            $min = floor($hour_data / 60); // 多少分钟
        }else if($data > 3600){
            $day = '00';
            $hour = floor($data / 3600); // 多少小时
            $hour_data = $data % 3600;
            $min = floor($hour_data / 60); // 多少分钟
        }else{
            $day = '00';
            $hour = '00';
            $min = floor($data / 60); // 多少分钟
        }
        // 补前导零
