...
class LessonVodModel extends PublicModel
{
    /* 会员模型自动验证 */
    ...
    public function lqGetField($sql, $field)
    {
        ...
        if ($field == "*") {
            return $this->where($sql . ' and zl_visible = 1')->field("*")->find();
        } else {
            if (count($field_arr) > 1) {
                return $this->where($sql . ' and zl_visible = 1')->field($field)->find();
            } else {
                return $this->where($sql . ' and zl_visible = 1')->getField($field);
            }
        }
    }
    ////通过条件获取字段(select)
    public function lqGetField_select($sql, $field)
    {
        $field_arr = split(',', $field);
        if ($field == "*") {
            return $this->where($sql . ' and zl_visible = 1')->field("*")->select();
        } else {
            if (count($field_arr) > 1) {
                return $this->where($sql . ' and zl_visible = 1')->field($field)->select();
            } else {
                return $this->where($sql . ' and zl_visible = 1')->getField($field);
            }
        }
    ...
    public function getCount($where = ''){
        if($where) return $this->where($where . ' and zl_visible = 1')->count();
    ...
    // 获取录播id
    public function get_vod_id($vod_id)
    {
        $vod_info = $this->where('id = '.$vod_id)->field('zc_vod_info')->find();
        if($vod_info){
            return end(explode('|||',$vod_info));
        }else{
            return false;
        }
    }
