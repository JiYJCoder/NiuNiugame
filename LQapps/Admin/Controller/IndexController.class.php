...
class IndexController extends PublicController{
	...
	protected function getDataFollow($date='2017-01-01',&$list){
		...
		$new=$this->model_member->apiFollowCount("zl_type=1 and zn_cdate >=".$start_time." and zn_cdate<=".$end_time);
		...
		$page_auth=$this->model_member->apiFollowCount("zl_type=2 and zn_cdate<=".$end_time);
		$member=$this->model_member->apiListCount("zn_cdate >=".$start_time." and zn_cdate<=".$end_time);
		
		$list["label"][] =$date;
		$list["datasets"]["new"][]=	$new;	
		$list["datasets"]["cancel"][]=	$cancel;	
		$list["datasets"]["increase"][]= $new-$cancel;	
		$list["datasets"]["cumulate"][]= $cumulate;		
	...
	public function modifyMyself() {
        ...
        if (IS_POST) {
			...
			foreach ($this->login_admin_info as $lnKey => $laValue) {$form_array[$lnKey]=$laValue;}
