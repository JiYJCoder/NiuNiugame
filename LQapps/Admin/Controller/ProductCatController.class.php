...
class ProductCatController extends PublicController{
	...
	protected $myForm = array(
		...
		//通用信息
		'1'=>array(
		array('buttonDialog', 'zn_fid', "父级分类",1,'{"required":"0","dataLength":"","readonly":1,"disabled":0,"controller":"ProductCat","type":"tree","checkbox":"0"}'),
    ...
    public function tree() {
		...
		$list=F('product_cat','',COMMON_ARRAY);
    ...
    public function opProperty() {
		...
		$product = M("product")->where("zn_cat_id=".$oid)->select();
    ...
    public function ajaxGetCount() {
		$tree = new \LQLibs\Util\Category('product_cat', array('id', 'zn_fid', 'zc_caption'));
		...
		$count = M("product")->where($sqlwhere_parameter)->count();
