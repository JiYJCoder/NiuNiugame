...
class LessonVodController extends PublicController
{
    public $myTable;
    protected $myForm = array(
        //标题
        'tab_title' => array(1 => '课节信息'),
        //通用信息
        '1' => array(
            array('text', 'zc_title', "课程标题", 1, '{"required":"1","dataType":"","dataLength":"","readonly":0,"disabled":0,"maxl":120}'),
            array('date', 'zc_date', "上传时间", 1, '{"required":"1","dataType":"","dataLength":"","readonly":0,"disabled":0,"format":"day"}'),
            array('text', 'zc_vod_info', "视频ID", 1, '{"required":"1","dataType":"","dataLength":"","readonly":0,"disabled":0,"format":"day"}'),
        )
    );
    ...
    public function index()
    {
		...
		if ($search_content_array["open_time"] == 1 && $search_content_array["time_start"] && $search_content_array["time_end"]) {
            ...
            $sqlwhere_parameter .= " and zn_date >='" . $ts . "' and zn_date<='" . $te."'";
        }
		...
		$page_config = array(
            'field' => "`id`,`zn_cat_id`,`zc_title`,`zn_cdate`,`zl_visible`",
            'where' => $sqlwhere_parameter,
            'order' => 'zn_cdate,id ASC',
            'title' => $page_title,
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            $form_array["zc_vod_info"] = end(explode("|||",$data['zc_vod_info']));
            $Form = new Form($this->myForm, $form_array, $this->myTable->getCacheComment());
            $this->assign("LQFdata", $Form->createHtml());//表单数据
            //表单数据初始化s
    ...
    {
        $vod_id = I("get.tnid");
        if(!$vod_id)  $this->error("参数丢失");

        $vodInfo = M("LessonVod")->where("id=".$vod_id)->getField('zc_vod_info');
        if(!$vodInfo) $this->error("视频信息丢失或视频已删除");
        /////调用阿里云视频接口
        $api = new vodApi();
        //////通过视频ID，由阿里云获取视频信息
        $aliVod = $api->GetPlayInfo(end(explode("|||",$vodInfo)));

        $this->assign("vod",getVodInfo($aliVod));
