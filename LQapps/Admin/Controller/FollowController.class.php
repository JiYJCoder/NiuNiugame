...
class FollowController extends PublicController{
    ...
    public function index() {
		...
		$this->assign("type_str",lqCreatOption(array(1=>'关注',2=>'页面授权'),$search_content_array["type"],"请选择入库类型"));
		...
		if($search_content_array["type"]){
			if ( $search_content_array["type"]==1 ){
				$sqlwhere_parameter.=" and zl_type =1 ";
			}else{
				$sqlwhere_parameter.=" and zl_type =2 ";
			}
		}
		if($search_content_array["sex"]!=''){
				$sqlwhere_parameter.=" and zn_sex =".intval($search_content_array["sex"]);
		}
		if($search_content_array["use"]!=''&&$search_content_array["type"]==1){
		...
		$page_config = array(
				'field'=>"`id`,`zl_type`,`zc_openid`,`zn_member_id`,`zc_member_account`,`zn_sex`,`zc_nickname`,`zc_headimg_url`,`zn_subscribe_time`,`zn_unsubscribe_time`,`zc_country`,`zc_province`,`zc_city`,`zl_visible`",
