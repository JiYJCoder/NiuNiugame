...
class AdminLogController extends PublicController{
    ...
    public function index() {
		...
		//首页设置
		$page_title=array('checkbox'=>'checkbox','no'=>L("LIST_NO"),'zn_cdate'=>'操作时间','zn_ip'=>'操作IP','zc_description'=>'标题','os'=>L("LIST_OS"));
		$page_config = array(
				'field'=>"`id`,`zc_description`,`zc_action`,`zn_ip`,`zn_cdate`",
        ...
        $count = $this->UserModel->apiLogCount($sqlwhere_parameter);
		$page = new \LQLibs\Util\Page($count, C("PAGESIZE"), $page_parameter);//载入分页类
        $showPage = $page->admin_show();
        $this->assign("page", $showPage);
        $this->assign("list", $this->UserModel->apiLogList($page->firstRow, $page->listRows,$page_config));
    ...
    public function opDeleteCheckbox() {
        $this->ajaxReturn($this->UserModel->apiDeleteLog());
    }
