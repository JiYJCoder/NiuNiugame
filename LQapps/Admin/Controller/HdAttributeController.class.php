<?php //家装属性类别 HdAttribute 页面操作 
namespace Admin\Controller;
use Think\Controller;
use LQLibs\Util\BootstrapForm as Form;//表单填充插件

class HdAttributeController extends PublicController{
	public $myTable;
	protected $myForm = array(
		...
		//基本信息
		'1'=>array(		
		array('select', 'zn_type', "属性类别",1,'{"required":"1","dataType":"","dataLength":"","readonly":0,"disabled":0,"please":"请选择"}'),
		array('text', 'zc_caption', "类别名称",1,'{"required":"1","dataType":"","dataLength":"","readonly":0,"disabled":0}'),
    ...
    public function index() {
		...
		if($search_content_array["type"])	$sqlwhere_parameter.=" and zn_type = ".$search_content_array["type"];
		
		//首页设置
		$page_title=array('checkbox'=>'checkbox','no'=>L("LIST_NO"),'id'=>L("LIST_ID"),'zn_type'=>'属性类别','zc_caption'=>'属性名称','zn_sort'=>L("LIST_SOTR"),'status'=>L("LIST_STAYUS"),'os'=>L("LIST_OS"));
		$page_config = array(
				'field'=>"`id`,`zn_type`,`zc_caption`,`zn_sort`,`zl_visible`",
    ...
    public function add() {
        if (IS_POST) {
			...
			$form_array["id"]='';
			$form_array["zn_type_data"]=C('HD_TYPE');
