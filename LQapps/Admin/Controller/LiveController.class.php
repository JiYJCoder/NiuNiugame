...
class LiveController extends PublicController
{
    ...
    public function index()
    {
        ...
        if ($search_content_array["status"]) {
            ...
            $this->assign("sys_heading", C('LIVE_STATUS')[$search_content_array["status"]]);
        }
        ...
        $page_config = array(
            'field' => "`id`,`zn_fid`,`zn_cat_id`,`zn_teacher_id`,`zc_image`,`zc_title`,`zl_is_index`,`zl_is_good`,`zc_file`,`zl_status`",
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            $form_array["zl_status_data"] = C('LIVE_STATUS');
    ...
    public function apply_window()
    {
        if (IS_POST) {
            ...
            if ($status == 2) $this->myTable->where("id=" . $lesson)->setField("zl_status", 1);
    ...
    public function ajaxSearchData()
    {
        ...
        $list['live_finish'] = M("Live")->where("zl_status = 1")->count();
        $list['live_closed'] = M("Live")->where("zl_status = 2")->count();
        $list['live_apply'] = M("Live")->where("zl_status = 4")->count();
        $list['teacher_total'] = count(M("Live")->group("zn_teacher_id")->select());
        $list['student_total'] = count(M("LiveRecord")->group("zn_member_id")->select());
        $list['vod_total'] = intval(M("LessonLive")->where("zc_vod_url <> ''")->count());
