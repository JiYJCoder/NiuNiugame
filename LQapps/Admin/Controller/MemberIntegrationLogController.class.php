...
class MemberIntegrationLogController extends PublicController{
    /** 初始化*/
    ...
    public function index() {
		...
		if($search_content_array["fkeyword"]&&$search_content_array["fkeyword"]!=$this->keywordDefault){
			...
			$sqlwhere_parameter.=" and (zc_type like'".$search_content_array["fkeyword"]."%' or zc_member_account like'".$search_content_array["fkeyword"]."%') ";
			}	
		...
		//首页设置
		$page_title=array('checkbox'=>'checkbox','no'=>L("LIST_NO"),'zn_cdate'=>'操作时间','zc_type'=>'积分行为','account'=>'帐号','zn_integration'=>'分值');
		$page_config = array(
				'field'=>"`id`,`zc_type`,`zc_member_account`,`zn_integration`,`zn_cdate`",
        ...
        $count = $this->model_member->apiIntegrationCount($sqlwhere_parameter);
		$page = new \LQLibs\Util\Page($count, C("PAGESIZE"), $page_parameter);//载入分页类
        $showPage = $page->admin_show();
        $this->assign("page", $showPage);
        $this->assign("list", $this->model_member->apiIntegrationList($page->firstRow, $page->listRows,$page_config));
    ...
    public function opDeleteCheckbox() {
        $this->ajaxReturn($this->model_member->apiDeleteIntegration());
    }
