<?php //课程分类 LessonCat 数据处理，数据回调
namespace Admin\Model;
use Think\Model;
use LQLibs\Util\Category as Category;//树状分类

class SchoolCatModel extends PublicModel {
	/* 用户模型自动验证 */
    ...
    public function __construct() {
		...
		$this->pc_index_list =  "SchoolCat/index";//列表首页
	}
	//防止自己日自己
    ...
    public function lqCacheData($tnReturn=0){
		$condition='';
		$tree = new Category("school_cat", array('id', 'zn_fid', 'zc_caption'));
		...
		F('school_cat',$array,COMMON_ARRAY);
    ...
    public function lqList($condition=array()) {
		$treelist=F('school_cat','',COMMON_ARRAY);
		if(empty($treelist)){
