...
class MemberEnrollModel extends PublicModel {
    ...
    public function lqList($firstRow = 0, $listRows = 20, $page_config=array('field'=>'*','where'=>' 1 ','order'=>'`id` DESC')) {
		...
		foreach ($list as $lnKey => $laValue) {
			...
			if($laValue['zn_type'] == 1) $table = $tableLive;
            else $table = $tableVod;


            $title = $table->where("id=".$laValue['zn_object_id'])->getField('zc_title');


            $list[$lnKey]['title'] = $title;
            $list[$lnKey]['student_name'] = $student;
			$list[$lnKey]['zn_fid_label'] = $cat[$laValue["zn_fid"]];
			$list[$lnKey]['zn_cat_id_label'] = $cat[$laValue["zn_cat_id"]];
