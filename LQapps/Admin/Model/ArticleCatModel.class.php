<?php //文章分类 ArticleCat 数据处理，数据回调
namespace Admin\Model;
use Think\Model;
use LQLibs\Util\Category as Category;//树状分类

class ArticleCatModel extends PublicModel {
	/* 用户模型自动验证 */
	...
	protected $_auto = array(
		array('zc_caption_alias', 'lqNull', self::MODEL_BOTH,'function'),
    ...
    public function __construct() {
		...
		$this->pc_index_list =  "ArticleCat/index";//列表首页
	}
	//防止自己日自己
    ...
    public function lqCacheData($tnReturn=0){
		...
		F('article_cat',$array,COMMON_ARRAY);
    ...
    public function lqList($condition=array()) {
		$treelist=F('article_cat','',COMMON_ARRAY);
		if(empty($treelist)){
    ...
    public function lqVisible($isTree=1) {
        ...
        if ($this->save($data)) {
			...
			if($data["zl_visible"]==0){
				$update=M()->execute( "update __PREFIX__article set zl_visible=0 where zn_cat_id in($child_ids)");
			}			
