...
class MemberApi extends Api
{
     ...
     */
    public function apiMidAuth($mid, $auth)
    {
        return $this->model->midAuth($mid, $auth);
    }
     ...
     */
    public function apiLogin($username, $password, $authType, $type = 1, $sha1 = 1, $where = '')
    {
        return $this->model->login($username, $password, $authType, $type, $sha1, $where);
    }
     ...
     * $type 1=>学生  2=>老师
     */
    public function apiGetGrowInfo($client_id, $secret, $type = 1)
    {
        return $this->model->getGrowInfo($client_id, $secret, $type);
    }
     ...
     *
     */
    public function apiGetGrowSchoolList()
    {
        return $this->model->getGrowSchollList();
    }

    /**
     * 会员登录信息写入session
     */
    public function apiLoginSession($mid, $type = 2, $app = false)
    {
        if ($type == 2){
            $authType = 'teacher';
            $clearType = 'student';
        ...
        if ($app == false) {
            session($authType . '_auth', $auth);
            session($authType . '_auth_sign', $this->dataAuthSign($auth));
            session($clearType . '_auth', null);
            session($clearType . '_auth_sign', null);
        } else {
    ...
    {
        session('student_auth', null);
        session('student_auth_sign', null);
        session('teacher_auth', null);
        session('teacher_auth_sign', null);
        return array('status' => 1, 'msg' => C('LQ_MEMBER_LOG')["login_out"] . "成功", 'url' => U('login'));
    }
    ...
    //注册一个新会员(后台新增)
    public function apiInsertMember()
    {
        return $this->model->lqInsertMember();
    }

    //更新会员表
    public function apiUpdateMember($data)
    {
        return $this->model->lqUpdateMember($data);
    }

    public function apiSaveMember($data)
    {
        return $this->model->lqSaveMember($data);
    }

    //会员积分更新
    public function apiUpdateIntegration($mid, $integration = 1, $is_add = 1)
    {
        return $this->model->lqUpdateIntegration($mid, $integration, $is_add);
    }

    //更新密码
    public function apiEditPass()
    {
        return $this->model->lqEditPass();
    }

    //插入积分
    public function apiInsertIntegration($key, $member)
    {
        return $this->model->lqInsertIntegration($key, $member);
    }

    //判断是否允许积分
    public function apiIsAllowIntegration($key, $member)
    {
        return $this->model->lqIsAllowIntegration($key, $member);
    }

    //喜欢管理
    public function apiTestLove($id, $key, $member)
    {
        return $this->model->lqTestLove($id, $key, $member);
    }

    public function apiInsertLove($id, $key, $member)
    {
        return $this->model->lqInsertLove($id, $key, $member);
    }

    public function apiDeleteLove($id, $key, $member)
    {
        return $this->model->lqDeleteLove($id, $key, $member);
    }

    public function apiGetFavoriteIds($type, $member)
    {
        return $this->model->laGetFavoriteIds($type, $member);
    }
    ...
    //会员积分总数
    public function apiIntegrationCount($sqlwhere_parameter)
    {
        return $this->model->lqIntegrationCount($sqlwhere_parameter);
    }
    ...
    /*会员其他#######################e*/
    /////收藏  type 1=> 直播  2=>点播
    public function apiMemberFavorite($objID, $type = 1)
    {

        return $this->model->memberFavorite($objID, $type);
    }

    ////删除收藏
    public function apiDelMemberFavorite($objID, $type = 1)
    {
        return $this->model->delMemberFavorite($objID, $type);
    }

    /////报名课程
    public function apiMemberEnroll($objID, $type = 1)
    {
        return $this->model->memberEnroll($objID, $type);
    }

    ////删除收藏
    public function apiDelMemberEnroll($objID, $type = 1)
    {
        return $this->model->delMemberEnroll($objID, $type);
    }

    /////关键字获取老师集合
    public function apiGetIdsByKeyword($kw, $school = 0)
    {
        return $this->model->getIdsByKeyword($kw, $school);
    }
     ...
     * $teacher_id   老师id
     */
    public function apiGetTeacherInfo($teacher_id)
    {
        return $this->model->getTeacherInfo($teacher_id);
    }
