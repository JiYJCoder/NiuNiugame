...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
		if(C("MODULE_ID")==2){
			...
			if(HIDDEN_INDEX_FILE==1){
				$url        =   '/'.INDEX_FILE_NAME.'?'.C('VAR_MODULE')."={$module}&".http_build_query(array_reverse($var));
			}else{
				$url        =   __APP__.'?'.C('VAR_MODULE')."={$module}&".http_build_query(array_reverse($var));
			}
		}
...
function F($name, $value='', $path=DATA_PATH) {
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            Think\Storage::put($filename,$f_cache_pre.serialize($value),'F');
            // 缓存数据
            $_cache[$name]  =   $value;
            return null;
        }
    ...
    if (isset($_cache[$name])){
        return str_replace($f_cache_pre,"",$_cache[$name]);;
	}
    if (Think\Storage::has($filename,'F')){
        $value      =   unserialize( str_replace($f_cache_pre,"",Think\Storage::read($filename,'F')) );
        $_cache[$name]  =   $value;
    } else {
        $value          =   false;
    }
