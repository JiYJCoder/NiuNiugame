...
class  Template {
    ...
    public function loadTemplate ($templateFile,$prefix='') {
        if(is_file($templateFile)) {
            ...
            // 读取模板文件内容
            $LQ_TMPL_CONTENT =  file_get_contents($templateFile);
        }else{
            $LQ_TMPL_CONTENT =  $templateFile;
        }
         // 根据模版文件名定位缓存文件
        ...
        // 判断是否启用布局
        if(C('LAYOUT_ON')) {
            if(false !== strpos($LQ_TMPL_CONTENT,'{__NOLAYOUT__}')) { // 可以单独定义不使用布局
                $LQ_TMPL_CONTENT = str_replace('{__NOLAYOUT__}','',$LQ_TMPL_CONTENT);
            }else{ // 替换布局的主体内容
                ...
                $LQ_TMPL_CONTENT = str_replace($this->config['layout_item'],$LQ_TMPL_CONTENT,file_get_contents($layoutFile));
            }
        }
        // 编译模板内容
        $LQ_TMPL_CONTENT =  $this->compiler($LQ_TMPL_CONTENT);
        Storage::put($tmplCacheFile,trim($LQ_TMPL_CONTENT),'tpl');
     ...
     * 编译模板文件内容
     * @access protected
     * @param mixed $LQ_TMPL_CONTENT 模板内容
     * @return string
     */
    protected function compiler($LQ_TMPL_CONTENT) {
        //模板解析
        $LQ_TMPL_CONTENT =  $this->parse($LQ_TMPL_CONTENT);
        // 还原被替换的Literal标签
        $LQ_TMPL_CONTENT =  preg_replace_callback('/<!--###literal(\d+)###-->/is', array($this, 'restoreLiteral'), $LQ_TMPL_CONTENT);
        // 添加安全代码
        $LQ_TMPL_CONTENT =  '<?php if (!defined(\'THINK_PATH\')) exit();?>'.$LQ_TMPL_CONTENT;
        // 优化生成的php代码
        $LQ_TMPL_CONTENT = str_replace('?><?php','',$LQ_TMPL_CONTENT);
		
		
		//修正模板路径  @theone 20160705 s
		if(C('TPL_WEBSITE_USE')) {
		$LQ_THEME_PATH=THEME_PATH;
		$LQ_THEME_PATH=str_replace("./",'/',$LQ_THEME_PATH);		
			$patterns = array();
			$patterns[0] = '/<\/title>/i';
			$patterns[1] = '/(<link\srel=["|\']stylesheet["|\']\stype=["|\']text\/css["|\']\shref=["|\'])(?:\.\/|\.\.\/)?(res\/*\/[\/a-z0-9A-Z_\-\.]+\.css(|\?[[a-z0-9A-Z_=.]+)["|\'])/i';
			$patterns[2] = '/(<script\s(?:type|language)=["|\']text\/javascript["|\']\ssrc=["|\'])(?:\.\/|\.\.\/)?(res\/*\/[\/a-z0-9A-Z_\-\.]+\.js(|\?[[a-z0-9A-Z_=.]+)["|\']|charset="utf-8"><\/script>)/';
			$patterns[3] = '/src="..\/res\/images\//i';
			$patterns[4] = '/..\/res\/images\/favicon.ico/i';
			$replacements = array();
			$replacements[0] = "</title>\r\n\t<meta name=\"author\" content=\"" . C('PROJECT_NAME') . "\" />";
			$replacements[1] = '\1' . $LQ_THEME_PATH . '\2';
			$replacements[2] = '\1' . $LQ_THEME_PATH . '\2';
			$replacements[3] = 'src="'.$LQ_THEME_PATH.'res/images/';
			$replacements[4] = $LQ_THEME_PATH.'res/images/favicon.ico';
			$LQ_TMPL_CONTENT=preg_replace($patterns, $replacements, $LQ_TMPL_CONTENT);
		}		
		//修正模板路径 e
		
        // 模版编译过滤标签
        Hook::listen('template_filter',$LQ_TMPL_CONTENT);
        return strip_whitespace($LQ_TMPL_CONTENT);
    }
