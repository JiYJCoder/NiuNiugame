...
class Model {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                ...
                $fields = F('_theone/'.strtolower($db.'.'.$this->tablePrefix.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
		...
		//注释列表
		$la_column_comment=$this->lqColumnComment();
        foreach ($fields as $key=>$val){
            // 记录字段类型
            $type[$key]     =   $val['type'];
			// 记录字段注释
			$comment[$key]     =   $la_column_comment[$val['name']];
        ...
        // 记录字段注释信息
        $this->fields['_comment'] =  $comment;
		
        // 2008-3-7 增加缓存开关控制
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            F('_theone/'.strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields);
        }
    }
	
	//获取数据表 字段注释 theone 2015-01-10 add
	public function lqColumnComment() {
		$db   =  $this->dbName?:C('DB_NAME');
		$comment_array=array();
		$column_comment=$this->query("select COLUMN_NAME,COLUMN_COMMENT,character_maximum_length FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='".$this->getTableName()."' and TABLE_SCHEMA='".$db."'");
		 foreach ($column_comment as $lnKey => $laValue) {
			 $comment_array[$laValue["column_name"]]=$laValue["column_comment"]."|theone|".$laValue["character_maximum_length"];
		 }
	...
	//获取数据表 字段注释 theone 2015-01-10 add
	public function getCacheComment() {
		$db   =  $this->dbName?:C('DB_NAME');
     ...
     */
     protected function _facade($data) {
		 //后追加 by @ theone 20160517 s
		 if(ACTION_NAME=='add'|ACTION_NAME=='edit'){
			if(empty($this->fields)) {
				$db   =  $this->dbName?:C('DB_NAME');
				$this->fields=F('_theone/'.$db.'.'.$this->tablePrefix.CONTROLLER_TO_TABLE($this->name));
			}
		 }//后追加 by @ theone 20160517 e	
    ...
    public function regex($value,$rule) {
        $validate = array(
            ...
            'double'    =>  '/^[-\+]?\d+(\.\d+)?$/',
            'english'   =>  '/^[A-Za-z]+$/',
			'lqrequire'   =>'/^([1-9]\d*)$/',   //theone
			'mobile'   =>  '/^1[34578]{1}\d{9}$/', 
			'zipcode'   =>  '/^[1-9]\d{5}$/',
			'chinese'   =>  '/^[\u0391-\uFFE5]+$/',
			'tel_area_code'   =>  '/^0\d{2,3}$/',
			'tel'   =>  '/^([0-9]{3,4}-)?[0-9]{7,8}$/',
        );
        // 检查是否有内置的正则表达式
     ...
     * @return \Think\Model
     * @auth byron sampson
	 * $user = M('Member')->master(true)->find();
     */
