    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
				...
				if(top.php_url_model==1){
                var actionUrl = top.LQ_DOMAIN+'sys-index.php/'+top.document.getElementById("my_controller_name").value+'/opUploadEditor';
				}else{
                var actionUrl = top.LQ_DOMAIN+'do?g='+(top.document.getElementById("my_module").value).toLowerCase()+'&m='+(top.document.getElementById("my_controller_name").value).toLowerCase()+'&a=op_upload_editor';
				}
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        ...
                        if(top.php_url_model==1){
                        var url = utils.formatUrl(actionUrl + '/encode/utf-8&' + params);
						}else{
    ...
    SearchImage.prototype = {
        ...
        getImageData: function(){
            var _this = this,
				...
				key = top.base64encode(key);
				
				if(top.php_url_model==1){
					var  url = top.LQ_DOMAIN+"sys-index.php/Attachment/editorImages" + "/keyword/"+key+"/d/" + +new Date;
				}else{
					var url = top.LQ_DOMAIN+'do?g='+(top.document.getElementById("my_module").value).toLowerCase()+'&m=attachment&a=editor_images&keyword='+key+"&d="+new Date;
				}				
                ...
                'onsuccess':function(r){
                    ...
                    if (json.state == 'SUCCESS') {
                        for(var i = 0; i < json.list.length; i++) {
                            if(json.list[i].url) {
								mylist.push({title: json.list[i].title,src: json.list[i].url,url: json.list[i].url});
                            }
                        }
                    }
                    _this.setList(mylist);
