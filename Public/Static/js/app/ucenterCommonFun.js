...
var php_url_model=0;//URL访问模式
var LQ_DOMAIN="http://"+window.location.host+"/"; //域名
var LQ = {
	EDITOR: LQ_DOMAIN+"do?/##/op_upload_editor/uid/###/token/####",
};
		...
		require(['layer'], function(){
			layer.confirm('确认要批量删除吗?',function(){
				...
				if(itemsTxt){
						...
						var thinkphpurl=$("#thinkphpurl").val()+"&a=op_delete_checkbox";
	...
	util.ajaxEdit = function(obj,url){
		...
		$("#edit_"+lqop+"_"+lqid).focus().bind("blur",function(){
				...
				if(lqval!=editval){
					...
					page_loading = layer.load(0,{shade:[0.1,'#fff']});
					if(lqop=='label'){
					$.post(url,{tnid:lqid,vlaue:editval},function(json){layer.close(page_loading);if(json.status==0) {util.sysMsg(0,json.msg);obj.html(lqval);}});
					}else if(lqop=='sort'){
					$.post(url,{tnid:lqid,vlaue:editval},function(json){layer.close(page_loading);if(json.status==0){util.sysMsg(0,json.msg);obj.html(lqval);}});
					}else{
					$.post(url,{tcop:lqop,tnid:lqid,vlaue:editval},function(json){layer.close(page_loading);if(json.status==0){util.sysMsg(0,json.msg);obj.html(lqval);}});
					}
					});
				}
		})	
