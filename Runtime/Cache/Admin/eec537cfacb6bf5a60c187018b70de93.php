            ...
            $(function(){
                ...
                $("#listSearch").click(function(){	
                    require(['layer'], function(){
						...
						var bind=$("#bind").val();
						var use=$("#use").val();
						if(fkeyword!='<?php echo ($keywordDefault); ?>'||role||bind||use||open_time==1){
            ...
            <table class="table table-hover">
              ...
              <tbody id="list-tbody">
                  <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty_msg" ;else: foreach($__LIST__ as $key=>$data): $mod = ($key % 2 );++$key;?><tr id="<?php echo ($data["id"]); ?>" visible="<?php echo ($data["zl_visible"]); ?>" opCheck='1' msg='1'>
                      ...
                      <a href="javascript:myConsume(<?php echo ($data["id"]); ?>)" title="查看消费明细"><i class="fa fa-list-alt"></i></a>
                      <a href="javascript:myRoom(<?php echo ($data["id"]); ?>)" title="查看房间列表"><i class="fa fa-desktop"></i></a>
