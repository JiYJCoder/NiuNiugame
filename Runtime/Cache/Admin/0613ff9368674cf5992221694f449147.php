        ...
        <div class="panel panel-default" id="scroll" style="margin-top:20px;">
            ...
            <div class="account-stat">
                <div class="account-stat-btn">
                    <div>直播申请<a href="<?php echo U('index',array('status'=>4));?>"><span id="live_apply">0</span></a></div>
                    <!--<div>直播总量<span id="live_total">0</span></div>-->
                    <div>生效直播<a href="<?php echo U('index',array('status'=>6));?>"><span id="live_effect">0</span></a></div>
                    <div>完结直播<a href="<?php echo U('index',array('status'=>1));?>"><span id="live_finish">0</span></a></div>
                    <div>下架直播<a href="<?php echo U('index',array('status'=>2));?>"><span id="live_closed">0</span></a></div>
            ...
            $(function(){
                ...
                var url = '<?php echo U("Live/ajaxSearchData");?>';
                $.post(url, function(data){
                    $("#live_effect").html(data.live_effect);
                    $("#live_finish").html(data.live_finish);
                    $("#live_closed").html(data.live_closed);
                    $("#live_apply").html(data.live_apply);
            ...
            <table class="table table-hover">
                 <thead>
                      <tr>
                        ...
                        <th style="width:150px;">附件数	</th>
                        <th style="width:120px;">直播状态</th>
              ...
              <tbody id="list-tbody">
                  <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty_msg" ;else: foreach($__LIST__ as $key=>$data): $mod = ($key % 2 );++$key;?><tr id="<?php echo ($data["id"]); ?>" visible="<?php echo ($data["zl_visible"]); ?>" opCheck='0'>
                      ...
                      <!--</td>                     -->
                      <td><a href="javascript:ossFile(<?php echo ($data["id"]); ?>)"> <?php echo ($data["file_num"]); ?></a></td>
                      ...
                      <a href="/sys-index.php/Live/edit/tnid/<?php echo ($data["id"]); ?>" title="<?php echo L("LABEL_OS_EDITID");?>[<?php echo ($data["id"]); ?>]"><i class="fa fa-edit"></i></a>
...
<script>
    ...
    function jumpUrl(id)
    {
        ...
        window.location.href = "/sys-index.php/LessonLive/index/s/"+urlpara;
    }
    function ossFile(id)
    {
        var urlpara =base64encode("zn_fid/"+id+"/");
        window.location.href = "/sys-index.php/OssFile/index/s/"+urlpara;
    }
</script>
...
<?php if(ACTION_NAME=='index') { echo "$('#list-tbody tr:odd').addClass('tr_odd');//单双行样式\n"; echo "$('.opStatus').click(function(){util.visible($(this),'/sys-index.php/Live/opVisible');});//快捷启用禁用操作\n"; echo "$('.opDelete').click(function(){util.delete($(this),'/sys-index.php/Live/opDelete');});//单记录删除操作\n"; echo "$('tbody>tr>td[op]').dblclick(function(){util.ajaxEdit($(this),'/sys-index.php/Live');});//单项编辑\n"; echo "$('tbody>tr>td>a[op]').click(function(){util.ajaxPropertyA($(this),'/sys-index.php/Live');});//单项属性切换\n"; } ?>	
	//ajax点击响应href
