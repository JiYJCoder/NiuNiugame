  ...
  <div class="clearfix welcome-container">
            ...
            $(function(){
                ...
                $("#listSearch").click(function(){	
                    require(['layer'], function(){
						...
						if(fkeyword!='<?php echo ($keywordDefault); ?>' || open_time==1){
							var searchurl=$("#thinkphpurl").val()+'index/s/';
							var urlpara="fkeyword/"+encodeURIComponent(fkeyword)+"/";
							urlpara+="keymode/"+encodeURIComponent($("#keymode").val())+"/";
	...
	<div class="panel-body">
        ...
        <thead class="navbar-inner">
          <tr>
            <?php if(is_array($page_config["title"])): $key = 0; $__LIST__ = $page_config["title"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$title): $mod = ($key % 2 );++$key; if($title == 'checkbox'): ?><th style="width:40px;"><input type="checkbox" class="checkbox" value="0" id="list_checkbox" /></th>
                <?php elseif(($title == '序号') OR ($title == 'ID')): ?>
                <th style="width:90px;text-align:center;"><?php echo ($title); ?></th>
                <?php elseif($title == '操作'): ?>
                <th style="width:200px;text-align:center;"><a href="javascript:;" id="op_msg"><?php echo ($title); ?>?</a></th>
                <?php elseif($title == '状态'): ?>
                <th style="width:50px;"><?php echo ($title); ?></th>
                <?php else: ?>
                <th><?php echo ($title); ?></th><?php endif; endforeach; endif; else: echo "" ;endif; ?>
          </tr>
        </thead>

              <tbody id="list-tbody">
                  <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty_msg" ;else: foreach($__LIST__ as $key=>$data): $mod = ($key % 2 );++$key;?><tr id="<?php echo ($data["id"]); ?>" visible="1" opCheck='0'>
                        <td align="center"><?php echo ($data["id"]); ?></td>
                        <td><?php echo ($data["title"]); ?></td>
                        <td><?php echo ($data["user"]); ?></td>
                        <td><?php echo ($data["zc_file_name"]); ?></td>
                        <td><?php echo ($data["size"]); ?></td>
                        <td><?php echo ($data["zn_cdate_label"]); ?></td>
                        <td class="manage-menu list-os-a">
                            <a href="/sys-index.php/OssFile/downlodFile/tnid/<?php echo ($data["id"]); ?>" title="下载附件"><i class="fa fa-download"></i></a>

                            <a href="javascript:;" class="delete" title="<?php echo L("LABEL_OS_TITLE_DEL"); echo ($data["title"]); ?>"><i class="fa fa-times-circle"></i></a>
                        </td>
...
<script>
    $(document).ready(function(){
        $('.delete').click(function(){
            var obj = $(this)
                require(['layer'], function () {
                    var url = "/sys-index.php/OssFile/deleteFile";
                    ...
                    layer.confirm(confirmStr, function () {
                        ...
                        $.getJSON(url, {tnid: id}, function (json) {
                            layer.close(page_loading);
                            if (json.status == 1) {
                                util.sysMsg(1, json.msg, 'ref');
                            } else {
...
<?php if(ACTION_NAME=='index') { echo "$('#list-tbody tr:odd').addClass('tr_odd');//单双行样式\n"; echo "$('.opStatus').click(function(){util.visible($(this),'/sys-index.php/OssFile/opVisible');});//快捷启用禁用操作\n"; echo "$('.opDelete').click(function(){util.delete($(this),'/sys-index.php/OssFile/opDelete');});//单记录删除操作\n"; echo "$('tbody>tr>td[op]').dblclick(function(){util.ajaxEdit($(this),'/sys-index.php/OssFile');});//单项编辑\n"; echo "$('tbody>tr>td>a[op]').click(function(){util.ajaxPropertyA($(this),'/sys-index.php/OssFile');});//单项属性切换\n"; } ?>	
	//ajax点击响应href
